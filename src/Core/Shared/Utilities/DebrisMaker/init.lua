...
local function SpawnSmallDebris(explosionPos, material, debrisCache, despawnTime, colour, size)
    ...
    for _ = 1, smallDebrisCount do
        ...
        angularVelocity.AngularVelocity =
            Vector3.new(random:NextNumber(-100, 100), random:NextNumber(-100, 100), random:NextNumber(-100, 100))
...
function DebrisMaker:Ground(Pos, Normal, Distance, Size, filter, MaxRocks, Ice, despawnTime, bigExplosion)
    ...
    local function InnerRocksLoop()
        for _ = 1, MaxRocks do
            local newCF = cf
                * CFrame.fromEulerAnglesXYZ(0, 0, math.rad(angle))
                * CFrame.new(0, Distance / 2 + Distance / 10, -10)
            ...
            if ray then
                ...
                local GoalCF = part.CFrame
                    * CFrame.new(0, 0, -part.Size.Z * 0.35)
                    * CFrame.fromEulerAnglesXYZ(
                        random:NextNumber(-1, -0.35),
                ...
                TweenService:Create(
                    hoof,
                    TweenInfo.new(0.15),
                    { CFrame = GoalCF * CFrame.new(0, 0, -part.Size.Z / 2 - hoof.Size.Z / 2.1) }
                ):Play()
                ...
                -- Material handling
                if
                    ray.Material == Enum.Material.Concrete
                    or ray.Material == Enum.Material.Air
                    or ray.Material == Enum.Material.Wood
                    or ray.Material == Enum.Material.Neon
                    or ray.Material == Enum.Material.WoodPlanks
                then
    ...
    local function OuterRocksLoop()
        for _ = 1, MaxRocks do
            local newCF = cf
                * CFrame.fromEulerAnglesXYZ(0, 0, math.rad(angle))
                * CFrame.new(0, Distance / 2 + Distance / 2.7, -Size.Y * 2)
            local ray = workspace:Raycast(newCF.Position, newCF.LookVector * -20, params)
            angle += otherAngle
            if ray then
                local part = partCache:GetPart()
